...
2026-06-11 — AgDex.ai LISTED chain-signer in their Security category (their words: "fills a clear gap — most agent security tools target prompt injection, while chain-signer catches the on-chain drain patterns that slip through"). Listing: https://agdex.ai/?q=Chain%20Signer. Replied (simple+direct per Kelvin): production use = pre-sign safety layer in our autonomous trading system; no public case study or logo yet, will follow. Logo v1 APPROVED by Kelvin + emailed to AgDex 2026-06-11 (threaded reply, PNG attached, SVG offered). Remaining TODO: public case study writeup once chart-trader goes live.

2026-06-12 — ROOT CAUSE of the stalled distribution chain FOUND + FIXED (Kelvin's screenshot prompted the dig): registry/server.json never declared the MCP launch command, so every evaluator ran the default `chain-signer` CLI (usage text, exit) instead of `chain-signer-mcp` — Glama's "cannot be installed" and the missing quality score were CORRECT symptoms. Shipped v0.5.30 (string args — registry publisher rejected, masked by continue-on-error) then v0.5.31 (Argument objects — PyPI + registry both green). Verified live: registry latest 0.5.31 carries runtimeHint uvx + the -mcp command; published wheel completes the MCP handshake. PR #7298 commented with root cause + reopen plan. WATCH: Glama re-crawl -> quality score -> reopen/resubmit listing. LESSON banked: continue-on-error masked the 0.5.30 registry-publish failure (prior versions published fine — the registry was at 0.5.29) — check job conclusions, not run conclusions.
