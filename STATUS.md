...
2026-06-12 — ROOT CAUSE of the stalled distribution chain FOUND + FIXED (Kelvin's screenshot prompted the dig): registry/server.json never declared the MCP launch command, so every evaluator ran the default `chain-signer` CLI (usage text, exit) instead of `chain-signer-mcp` — Glama's "cannot be installed" and the missing quality score were CORRECT symptoms. Shipped v0.5.30 (string args — registry publisher rejected, masked by continue-on-error) then v0.5.31 (Argument objects — PyPI + registry both green). Verified live: registry latest 0.5.31 carries runtimeHint uvx + the -mcp command; published wheel completes the MCP handshake. PR #7298 commented with root cause + reopen plan. WATCH: Glama re-crawl -> quality score -> reopen/resubmit listing. LESSON banked: continue-on-error masked 28 versions of registry-publish failures — check job conclusions, not run conclusions.
