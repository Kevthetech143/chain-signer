...
## Glama (verified 2026-06-01): manual 'Add Server' is AUTH-GATED (logged-out click is a no-op; Sign Up required). Glama auto-indexes public GitHub MCP repos and supersets the official registry — we qualify (public repo + server.json + official-registry listing), so auto-index is the expected path. DECISION: rely on auto-index; do NOT force a Glama OAuth signup just for one listing. Re-check in a few days; if still absent, create a Glama account as a deliberate step. mcp.so submission (#2605) stands. Smithery next (likely also account-gated — same call).
